...
Each character has some amount of Health, which starts out at that character's Maximum Health. A character falls Unconscious when their Health is reduced to [0].

\pagebreak
\section{Actions}

Each action either takes some amount of \{time\} to perform, or has a \{trigger\} condition. On your turn, you may continue performing actions until you have acted for a total of [6] seconds. Actions with trigger conditions can be performed instantly whenever their trigger condition is met (even if it’s not your turn), and don’t require using your action time. However, only one action can be triggered by a single event; if you have multiple actions with the same trigger condition, you must choose one.
\\[5mm]
...
\item Hike/Climb/Swim \TIME{\X}: move [$\X / 2$] spaces over difficult terrain, along a climbable wall, or through water.

\item Ready \TIME{\X}: choose a trigger condition and a list of actions that takes at most [\X] seconds. The first time that the trigger occurs before the start of your next turn, all of your chosen actions occur as one triggered action. You may only have one action readied at a time.
...
Attacks have accuracy [\ACC] and do damage [\DAM]. To perform an Attack action, roll [$\dice{1}{20} + \ACC$]. This will trigger the target’s Defend actions, if they have any. If none of the Defend actions stop your Attack, the attack succeeds, and the target loses [\DAM] Health. Additionally, when you roll for the Attack action, there are two special outcomes. If the die lands on a 1, then the attack will deal no damage (a critical failure). However, if the die lands on a 20, then the attack will deal twice as much damage (a critical hit)!
\\[5mm]
The following actions may be performed by any character:
\begin{itemize}
...
\nameref{Berserker} &  &  & \nameref{Vanguard} & Chimera & \nameref{Cavalier} \\
 ...
 &  & Savant &  & Wizard &  \\
 &  &  & Deadeye &  &   \\
Sanguine & \nameref{Zephyr} &  &  & Primordial & Summoner \\
 &  & Noble & Mimic &  & \nameref{Fool} \\
\end{tabularx}
\\[5mm]
...
\begin{itemize}
\item Gain one level in a class of your choice. This will result in gaining a new action.
...
Separately, you can gain the first level of each class by triggering the Unlock condition for that class. When you do, you gain the first level of that class immediately. Triggering the Unlock condition does nothing if you've already gained the first level of that class.
...
{You may act for up to [\ACU] seconds during this action.}
...
\Skill{2}{Charge}{\TIME{1}}
{TODO: flavor: use momentum to put force behind an attack}
{Your next Melee Attack this turn does [$\min(\X, \STR)$] additional damage, where [\X] is the distance traveled while using this skill.}
...
\Skill{2}{Chekhov's Gun}{\{time: 1 hour\}}
{TODO: flavor}
{Publicly choose an object. If the object is not used for at least a day, then add your [$\FOR$] to the next roll made using the object. This ability cannot be used on the same type of object more than once, and using this ability again removes any benefits given to previous objects.}
